...
// actions
import {loadRooms, roomSelect} from './actions.js'
...
const RoomList = ({rooms, selectedRoomId, onSelect}) => {
  return (
    <div>
      {rooms.list.map(({_id, level, creator, people}) => {
        const isSelected = selectedRoomId == _id
        const onRoomSelect = () => onSelect(_id)
        return (
          <Room
            creator={creator}
...
class Rooms extends Component {
  ...
  componentDidMount() {
    superagent
      ...
      .end((err, res) => {
        ...
        this.props.room.loadRooms(results)
      })
  }
  ...
  onRoomSelect(id) {
    this.props.room.roomSelect(id)
  }

  filteredMessages() {
    return this.state.messages.filter(
      ({channel_id}) => channel_id === this.props.rooms.selected
    )
  }
      ...
      <Box>
        ...
        <Box height="500px" overflowy="scroll" overflowx="none">
          <RoomList
            ...
            selectedRoomId={this.props.rooms.selected}
...
const mapDispatchToProps = dispatch => {
  return {
    room: bindActionCreators({loadRooms, roomSelect}, dispatch)
  }
}
